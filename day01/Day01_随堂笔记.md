...
## 入门案例_汽车类
	 ...
	 面试
	类和对象概念：类是一个抽象概念，它里面是属于对属性和行为的一种封装。
				对象就是照着类做出来的一个实体，对象就是类的具体体现和实现。
...
**self关键字的意义，为什么需要self关键字**

python内置的关键字，用于指向对象实例
...
`c1` 和 `c2` 是**变量名（引用变量）**，存在于栈内存中，用来指向堆内存里的对象实例。
...
- 当你写 
  ...
  1. Python 会在**堆内存**里创建一个 `Car` 类的实例对象（地址 `0x01`）。
  2. 栈里的变量 `c1` 会保存这个对象的内存地址，相当于 “指针”，指向堆里的实例。

  

- 同理，`c2 = Car()` 会在堆里创建另一个 `Car` 实例（地址 `0x02`），栈里的 `c2` 保存这个地址。
